...
class ContactService {
    ...
    async getAllContacts() {
        ...
        const contacts = await this.client.invoke(new telegram_1.Api.contacts.GetContacts({
            hash: 0
        }));
        const result = {
            contacts: [],
        ...
        if (contacts.users) {
            contacts.users.forEach(user => {
                ...
                if (user.bot) {
                    result.bots.push({ ...contactInfo, isBot: true });
                }
                else {
    ...
    async searchContacts(query) {
        try {
            const result = await this.client.invoke(new telegram_1.Api.contacts.Search({
                q: query,
                limit: 100
            }));
            const contacts = [];
    ...
    formatContactsForDisplay(contacts) {
        console.log('\n=== Export Summary ===');
