...
import { DialogInfo, GroupMember, GroupMembersExport, ExportFormat } from '../models/types';
...
export class MemberService implements IMemberService {
  ...
  async getGroupMembers(group: DialogInfo, limit: number | null = null): Promise<GroupMember[]> {
  ...
  async exportGroupMembers(group: DialogInfo, format: ExportFormat = 'json', limit: number | null = null): Promise<string | null> {
    ...
    const membersExport: GroupMembersExport = {
      groupInfo: {
        id: group.id,
        title: group.title,
  ...
  async getMemberById(group: DialogInfo, userId: number): Promise<GroupMember | null> {
    try {
  ...
  async searchMembers(group: DialogInfo, query: string): Promise<GroupMember[]> {
  ...
  async getAdminMembers(group: DialogInfo): Promise<GroupMember[]> {
    const allMembers = await this.getGroupMembers(group);
    return allMembers.filter(member => 
  ...
  async getBannedMembers(group: DialogInfo): Promise<GroupMember[]> {
    const allMembers = await this.getGroupMembers(group);
    return allMembers.filter(member => member.role === 'banned');
  }

  private mapParticipantToMember(participant: any): GroupMember {
